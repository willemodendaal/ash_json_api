...
With the above configuration, the path to "get" your Tickets resource would be: `/api/json/helpdesk/tickets`:

- "/api/json", which comes from the `scope` in the router.
- "/helpdesk", which comes from the `forward` in the router.
